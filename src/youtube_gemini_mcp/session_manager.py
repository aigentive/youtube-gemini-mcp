...
class SessionManager:
    ...
    def create_session(
        ...
        try:
            with self._lock:
                ...
                logger.info(f"Created session {session.session_id} for {source_type}: {video_source}")
    ...
    def _save_session_metadata(self, session: VideoSession) -> None:
        ...
        try:
            ...
            for call in session_dict["analysis_calls"]:
                call["created_at"] = call["created_at"].isoformat() if isinstance(call["created_at"], datetime) else call["created_at"]
            
            with open(metadata_file, "w") as f:
                json.dump(session_dict, f, indent=2)
            
        except Exception as e:
            logger.error(f"Failed to save session metadata for {session.session_id}: {e}")
    ...
    def _save_conversation_log(self, session: VideoSession) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save conversation log for {session.session_id}: {e}")
    ...
    def _archive_session(self, session: VideoSession) -> None:
        ...
        try:
            ...
            if source_dir.exists():
                ...
                logger.info(f"Archived session {session.session_id} to expired directory")
